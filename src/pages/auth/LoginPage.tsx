...
import { Loader2, MessageCircleMore, ScanFace } from "lucide-react";

import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import FaceLoginDialog from "@/components/auth/FaceLoginDialog";
import { faceLogin, login } from "@/features/auth/auth.api";
import { App } from "antd";
import type { LoginRes } from "@/features/auth/auth.type";
...
export default function LoginPage() {
  ...
  const saveLoginData = (res: LoginRes) => {
    if (!res.accessToken) {
  ...
  const handleFaceLogin = async (file: Blob) => {
    try {
      setFaceLoading(true);
      setError("");

      const res = await faceLogin(file)
      ...
      const message =
        err?.response?.data?.message ||
        err?.response?.data?.error ||
        "Đăng nhập bằng khuôn mặt thất bại";
      notification.error({
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-3">
                ...
                <Button
                  ...
                  variant="outline"
                  className="w-full"
                  onClick={() => setOpenFaceDialog(true)}
                  disabled={loading || faceLoading}
                >
